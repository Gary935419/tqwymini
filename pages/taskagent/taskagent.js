...
Page({
    ...
    wx.request({
      ...
      success: function(res) {
        ...
        if (res.data.errcode == '200') {
		  ...
		  //商品描述富文本编辑器
		  WxParse.wxParse('article', 'html', that.data.setinfo.contentagent, that, 5);
  ...
  get_commission_list: function() {
    ...
    wx.request({
      url: app.taskapi + '/Index/commissionlist',
      method: 'post',
      data: {
        token: main.get_storage('token'),
      ...
      success: function(res) {
        if (!main.checklogin(res, 'commission')) {
          return;
        }
        if (!res.data) {
          wx.showToast({
        ...
        if (res.data.errcode == '200') {
		  ...
		  if (that.data.page == 1) {
		  			that.setData({
		  			  hidden: false,
		  			  hidden1: true,
		  			  list: res.data.data.commissionlist,
		  			})
		  } else {
		    if (res.data.data.commissionlist.length == 0 && that.data.page > 1) {
		  				if(that.data.list == ''){
		  					that.setData({
		  					  hidden: true,
		  					  hidden1: false,
		      ...
		      that.setData({
		  				  hidden: false,
		  				  hidden1: true,
		          list: that.data.list.concat(res.data.data.commissionlist),
		      })
		    }
		  }
        } else {
